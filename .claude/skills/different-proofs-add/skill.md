...
3. **Update README.md and the Verso Blueprint chapter** under `DifferentProofsBlueprint/Chapters/<Theorem>.lean`. README gets a bullet `- [<Name>'s proof](DifferentProofs/<Theorem>/<Name>.lean)`. The chapter gets the appropriate `:::theorem`/`:::lemma_`/`:::proof` blocks with stable labels and `(lean := "...")` links to the formal declarations.
