...
private lemma coeff_norm_sq (n : ℤ) :
  ...
  · have hn' : (n : ℝ) ≠ 0 := Int.cast_ne_zero.mpr hn
    ...
    rw [show ((π : ℂ) - ((-π : ℝ) : ℂ)) = ((2 * π : ℝ) : ℂ) from by push_cast; ring,
    ...
    have hna : |(n : ℝ)| ≠ 0 := abs_ne_zero.mpr hn'
    rw [hd, show 1 / (2 * π * |(n : ℝ)|) * (2 * π) = 1 / |(n : ℝ)| from by field_simp,
...
theorem BaselProblem_Parseval : BaselProblem := by
  ...
  rw [show (π - -π)⁻¹ * (2 * π ^ 3 / 3) = π ^ 2 / 3 from by field_simp; ring] at h2S
  linarith [h2S]
