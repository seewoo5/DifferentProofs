...
lemma isCompact_toSet (R : Rectangle) : IsCompact R.toSet :=
...
lemma aedisjoint_toSet_of_disjoint_interior {A B : Rectangle}
  ...
  -- The interior of a box is the product of the open coordinate intervals.
  simp only [Rectangle.toSet, interior_prod_eq, interior_Icc, Set.disjoint_prod] at h
  -- An empty open coordinate interval makes the corresponding closed-interval overlap null.
  have hzero : ∀ {a b c d : ℝ}, Disjoint (Ioo a b) (Ioo c d) →
  ...
  -- `AEDisjoint volume s t` is by definition `volume (s ∩ t) = 0`.
  have key : volume (A.toSet ∩ B.toSet) = 0 := by
    simp only [Rectangle.toSet, Set.prod_inter_prod]
    rw [Measure.volume_eq_prod ℝ ℝ, Measure.prod_prod]
    rcases h with hx | hy
    · rw [hzero hx, zero_mul]
    · rw [hzero hy, mul_zero]
  exact key
...
theorem IsTiling.setIntegral_eq_sum {ι : Type} [Fintype ι] {R : Rectangle} {T : ι → Rectangle}
  ...
  have hcover := hT.cover
  rw [hcover, integral_iUnion_ae (fun i => (T i).measurableSet_toSet.nullMeasurableSet)
    (fun i j hij => aedisjoint_toSet_of_disjoint_interior (hT.interiorDisjoint hij))
    (by rw [← hcover]; exact hf)]
...
theorem Rectangle.setIntegral_prod_mul (R : Rectangle) {L : Type*} [RCLike L] (g h : ℝ → L) :
  ...
  rw [Rectangle.toSet, Measure.volume_eq_prod ℝ ℝ]
  exact MeasureTheory.setIntegral_prod_mul g h _ _
...
theorem IsTiling.prod_integral_dichotomy {ι : Type} [Fintype ι] {R : Rectangle} {T : ι → Rectangle}
  ...
  have hadd : (∫ z in R.toSet, g z.1 * h z.2) = ∑ i, ∫ z in (T i).toSet, g z.1 * h z.2 :=
    hT.setIntegral_eq_sum hint
  have key : (∫ x in Icc R.x₀ R.x₁, g x) * (∫ y in Icc R.y₀ R.y₁, h y) = 0 := by
    rw [← R.setIntegral_prod_mul g h, hadd]
    refine Finset.sum_eq_zero fun i _ => ?_
    rw [(T i).setIntegral_prod_mul g h]
    rcases htile i with h1 | h1
    · rw [h1, zero_mul]
    · rw [h1, mul_zero]
