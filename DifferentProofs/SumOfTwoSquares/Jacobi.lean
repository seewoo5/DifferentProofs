...
/-- Every element of the `ℤ`-subalgebra of `ℂ` generated by `I` is a Gaussian integer. -/
private lemma exists_gaussianInt_eq {z : ℂ} (hz : z ∈ Algebra.adjoin ℤ ({I} : Set ℂ)) :
    ∃ g : GaussianInt, (g : ℂ) = z := by
  rw [Algebra.adjoin_int] at hz
  refine RingHom.mem_range.mp (Subring.closure_le.mpr ?_ hz)
  exact Set.singleton_subset_iff.mpr ⟨⟨0, 1⟩, by simp [GaussianInt.toComplex_def']⟩
...
theorem FermatSumOfTwoSquares_Jacobi : FermatSumOfTwoSquares := by
  ...
  obtain ⟨g, hg⟩ := exists_gaussianInt_eq
