...
theorem prod_of_inv_of_one_sub_inv_prime_ge_harmonic (n : ℕ) :
    ∏ p ∈ (Finset.range (n + 1)).filter Nat.Prime, (p : ℚ) / (p - 1) ≥ harmonic n := by
  -- By definition of harmonic, we know that
  -- $\prod_{p \le n, p \text{ prime}} (1 - 1 / p)^{-1} \geq \sum_{k=1}^{n} \frac{1}{k}$.
  have h_prod_ge_sum : (∏ p ∈ Finset.filter Nat.Prime (Finset.range (n + 1)), (∑ i ∈ Finset.range (Nat.log p n + 1), (1 / p : ℚ) ^ i)) ≥ (∑ i ∈ Finset.range n, (1 / (i + 1) : ℚ)) := by
    -- Every integer $k$ in the range $1$ to $n$ can be written as a product of prime powers $p^e$ where $p$ is a prime and $e$ is a non-negative integer.
    have h_factorization : ∀ k ∈ Finset.range n, ∃ f : ℕ → ℕ, (∀ p, Nat.Prime p → f p ≤ Nat.log p n) ∧ (∏ p ∈ Finset.filter Nat.Prime (Finset.range (n + 1)), p ^ f p) = k + 1 := by
      intro k hk
      use fun p => Nat.factorization (k + 1) p;
      refine' ⟨ fun p pp => Nat.le_log_of_pow_le pp.one_lt <| Nat.le_trans ( Nat.le_of_dvd ( Nat.succ_pos _ ) <| Nat.ordProj_dvd _ _ ) <| Nat.succ_le_of_lt <| Finset.mem_range.mp hk, _ ⟩;
      conv_rhs => rw [ ← Nat.factorization_prod_pow_eq_self ( by positivity : ( k + 1 ) ≠ 0 ) ] ;
      rw [ Finsupp.prod_of_support_subset ] <;> norm_num;
      exact fun p hp => Finset.mem_filter.mpr ⟨ Finset.mem_range.mpr ( Nat.lt_succ_of_le ( Nat.le_trans ( Nat.le_of_mem_primeFactors hp ) ( by linarith [ Finset.mem_range.mp hk ] ) ) ), Nat.prime_of_mem_primeFactors hp ⟩;
    choose! f hf₁ hf₂ using h_factorization;
    -- By definition of $f$, we can rewrite the sum $\sum_{k=1}^{n} \frac{1}{k}$ as $\sum_{k=1}^{n} \prod_{p \le n, p \text{ prime}} \frac{1}{p^{f(k, p)}}$.
    have h_sum_rewrite : ∑ k ∈ Finset.range n, (1 / (k + 1) : ℚ) = ∑ k ∈ Finset.range n, (∏ p ∈ Finset.filter Nat.Prime (Finset.range (n + 1)), (1 / p : ℚ) ^ f k p) := by
    ...
    rotate_left;
    exact Finset.image ( fun k => fun p hp => f k p ) ( Finset.range n );
    · grind;
    · rw [ Finset.sum_image ];
      · exact Finset.sum_le_sum fun _ _ => by rw [ ← Finset.prod_attach ] ;
      · intro k hk l hl hkl; have := hf₂ k hk; have := hf₂ l hl; simp_all +decide [ Finset.prod_eq_zero_iff, Nat.Prime.ne_zero ] ;
        have := hf₂ k hk; have := hf₂ l hl; simp_all +decide [ funext_iff ] ;
        have := hf₂ k hk; have := hf₂ l hl; simp_all +decide [ Finset.prod_congr rfl fun p hp => show p ^ f k p = p ^ f l p from by rw [ hkl p ( Finset.mem_range_succ_iff.mp ( Finset.mem_filter.mp hp |>.1 ) ) ( Finset.mem_filter.mp hp |>.2 ) ] ] ;
  ...
  · refine Finset.prod_le_prod ?_ ?_ <;> norm_num;
    ...
    · intro i hi hi'; have := geom_sum_mul ( i : ℚ ) ⁻¹ ( Nat.log i n + 1 ) ; simp_all +decide [ div_eq_mul_inv, mul_assoc, mul_comm, mul_left_comm ] ;
      rcases i with ( _ | _ | i ) <;> norm_num at *;
      nlinarith [ inv_pos.mpr ( by positivity : 0 < ( i : ℚ ) + 1 + 1 ), inv_pos.mpr ( by positivity : 0 < ( i : ℚ ) + 1 ), mul_inv_cancel₀ ( by positivity : ( i : ℚ ) + 1 + 1 ≠ 0 ), mul_inv_cancel₀ ( by positivity : ( i : ℚ ) + 1 ≠ 0 ), pow_pos ( by positivity : 0 < ( i : ℚ ) + 1 + 1 ) ( Nat.log ( i + 1 + 1 ) n + 1 ), inv_pos.mpr ( pow_pos ( by positivity : 0 < ( i : ℚ ) + 1 + 1 ) ( Nat.log ( i + 1 + 1 ) n + 1 ) ) ]
...
theorem InfinitudeOfPrimes_Euler : InfinitudeOfPrimes := by
  ...
  have hprod_eq : (Finset.range (max n N + 1)).filter Nat.Prime =
      (Finset.range (N + 1)).filter Nat.Prime := by
    ext p; simp only [Finset.mem_filter, Finset.mem_range]
    exact ⟨fun ⟨_, hp⟩ => ⟨Nat.lt_succ_of_le (hN p hp), hp⟩,
           fun ⟨hle, hp⟩ => ⟨by omega, hp⟩⟩
  rw [hprod_eq] at h1
  -- harmonic is monotone
  have h2 : harmonic n ≤ harmonic (max n N) :=
    Finset.sum_le_sum_of_subset_of_nonneg
      (Finset.range_mono (le_max_left n N))
